...
impl Initiator {
    ...
    pub fn handshake_initiation(&mut self) -> Result<Vec<u8>, Error> {
        ...
        let encrypted_static: EncryptedStatic =
            aead(&key, 0, self.static_public, self.hash)?.as_slice().try_into()?;
        ...
        let encrypted_timestamp: EncryptedTimestamp =
            aead(&key, 0, timestamp.to_bytes(), self.hash)?.as_slice().try_into()?;
    ...
    pub fn on_handshake_response(
        ...
        let mac2: Mac = match self.last_received_cookie.as_ref() {
            Some(cookie) => {
                keyed_blake2s(cookie, &data[..(HANDSHAKE_RESPONSE_LEN - MAC_LEN)])?.into()
            }
            None => Mac::zero(),
        };
        if mac2 != macs.mac2 {
...
impl Decode for u32 {
    fn decode_from_slice(slice: &[u8]) -> Result<(Self, &[u8]), Error> {
        ...
        Ok((u32::from_le_bytes(bytes), slice))
    }
}
...
mod tests {
    ...
    fn handshake() {
        ...
        let initiation = match message {
            Message::HandshakeInitiation(message, macs) => responder
        ...
        let _response = match message {
            Message::HandshakeResponse(message, macs) => initiator
